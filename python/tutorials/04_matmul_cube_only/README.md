...
本样例基于Ascend C Python工程，介绍了纯Cube模式（只有矩阵计算）的Matmul算子实现。
...
- 算子实现
  ...
  本样例中实现的是基于Ascend910B1平台（共24个核，每个核包含1个AIC核和2个AIV核），固定shape为[M, N, K] = [128, 64, 30720]的Matmul算子。通过设置Kernel核函数JIT编译参数matmul_cube_only=True定义纯Cube模式的Matmul算子，该参数含义见[architecture_introduction.md](../../../docs/architecture_introduction.md)编译和运行模块。
  - kernel实现
...
- 打开样例目录。
  ```
  cd pyasc/python/tutorials/04_matmul_cube_only
  ```
- 修改配置

  修改matmul_cube_only.py中USE_CORE_NUM参数为当前执行平台的实际Cube核数。
- 执行如下命令可进行功能验证
  ```
  python3 matmul_cube_only.py
  ```
  用例执行完成，打屏信息出现“Sample matmul_cube_only run success.”，说明样例执行成功。
