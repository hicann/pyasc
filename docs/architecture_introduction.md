...
- 模块功能概述
  ...
  编译和运行模块通过JIT(Just-In-Time，即时编译)机制拉起，开发者需要导入本项目顶层模块asc，用装饰器@asc.jit修饰Kernel函数，拉起整个Kernel函数的编译和运行。对于Kernel函数的入参，开发者可通过小括号()传入编译参数，通过中括号[]传入运行参数。
...
- Python接口使用注意事项
    ...
    - 常量表示
        ...
        在接口中，某些参数需要作为常量表达式传递，可使用`asc.ConstExpr[origin_type]`泛型标记，表明参数为常量。例如以下代码中`BUFFER_NUM`和`TILE_LENGTH`为常量。
    ...
    - async def语句
      ...
      - 示例
        ...
        async def async_task(name, delay):
            ...
            print(f"{name} end, took {delay} seconds")
            return delay
        ```
      ...
      - 示例
        ...
        async def main():
            async with AsyncFile() as f:
                print("Performing asynchronous file read/write...")
